...
#   /etc/home-router-panel/awg/custom_domains.txt
...
log "Резолвинг доменов custom из $CONF_DIR/custom_domains.txt..."
ensure_ipset custom_nets "hash:ip"
count=0
while IFS= read -r domain; do
    while IFS= read -r ip; do
        ipset add custom_nets "$ip" 2>/dev/null || true
        (( count++ )) || true
    done < <(getent ahostsv4 "$domain" 2>/dev/null | awk '{print $1}' | sort -u || true)
done < <(read_conf_lines "custom_domains.txt")
log "  custom_nets: $count IP"

iptables -t mangle -A "$CHAIN" -m set --match-set custom_nets dst -j MARK --set-xmark "$FWMARK/$FWMARK_MASK"
